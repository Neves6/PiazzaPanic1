...
public class TestOrderMaster {
    ...
    public void testCheckOrderExpiredTrue(){
        ...
        assertEquals("Reputation point timer has not updated when an order expired", currentTime, OrderMaster.checkOrderExpired(customers, currentTime, 1, 3, 1, 5), 0.0);
    }
    ...
    public void testCheckOrderExpiredFalse(){
        ...
        customers.add(new Customer("b", 5, 5, "test2", 2));
        float currentTime = 5;
        assertEquals("Reputation point timer has changed but an order has not expired", 400, OrderMaster.checkOrderExpired(customers, currentTime, 1, 3, 1, 400), 0.0);
    }

    @Test
    public void testCreateCustomersValid(){
        Queue<Customer> customers = new LinkedList<>();
