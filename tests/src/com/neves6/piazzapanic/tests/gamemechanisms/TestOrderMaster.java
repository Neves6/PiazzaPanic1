...
public class TestOrderMaster {
  ...
  public void testCheckOrderExpiredTrue() {
    ...
    assertEquals(
        "Reputation point timer has not updated when an order expired",
        currentTime,
        OrderMaster.checkOrderExpired(customers, currentTime, 1, new ReputationPoints(3), 1, 5),
        0.0);
  }
  ...
  public void testCheckOrderExpiredFalse() {
    ...
    float currentTime = 5;
    assertEquals(
        "Reputation point timer has changed but an order has not expired",
        400,
        OrderMaster.checkOrderExpired(customers, currentTime, 1, new ReputationPoints(3), 1, 400),
        0.0);
  }
  ...
  public void testCreateCustomersInvalid() {
    ...
    // stalled instead
    assertTrue(
        "Time of last customer creation should not have been updated",
        lastCustomer == 5 || lastCustomer == 3);
  }
}
