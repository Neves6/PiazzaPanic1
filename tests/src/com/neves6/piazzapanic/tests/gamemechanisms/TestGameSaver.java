...
public class TestGameSaver {
    ...
    public void testInvalidDifficultyLower(){
        assertFalse("Entering a value between 1 and 3 should be" +
                "the only valid number.", testSaver.setDifficulty(0));
    }

    @Test
    public void testInvalidDifficultyHigher(){
        assertFalse("Entering a value between 1 and 3 should be" +
                "the only valid number.", testSaver.setDifficulty(4));
    }

    @Test
    public void testAddNoCustomers(){
        assertFalse("Entering 0 or less customers is invalid",
    ...
    public void testDecrementCustomerEndless(){
        testSaver.setCustomersRemaining(-1);
        assertFalse("You cannot decrement customers when running in endless mode.",
    ...
    public void testAddNegativeReputationPoints(){
        assertFalse("You can not have negative reputation point during a running game",
                testSaver.setReputationPoints(-1));
    }

    @Test
    public void testCustomersNull(){
        assertFalse("Customers cannot be null when adding details",
                testSaver.setRecipe(null));
    }

    @Test
    public void testTimeNeg(){
        assertFalse("Time cannot be negative",
                testSaver.setTime(-1f));
    }
